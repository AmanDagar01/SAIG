    ...
    return (
      <div className="bg-bg-card border border-border-primary rounded-xl p-5 glow-border h-full">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base font-semibold text-text-primary">Intel Summary</h2>
        </div>
  
        {/* Escalation Index - Prominent */}
        <div className={`rounded-lg border p-4 mb-4 ${getEscalationBg(escalationIndex)}`}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-text-secondary uppercase tracking-wider">
              Escalation Index
            </span>
            <Thermometer className={`w-4 h-4 ${getEscalationColor(escalationIndex)}`} />
          </div>
          <div className={`text-3xl font-bold ${getEscalationColor(escalationIndex)}`}>
            {escalationIndex}
          ...
          <div className="w-full bg-bg-primary rounded-full h-2 mt-2">
            <div
              className={`h-2 rounded-full transition-all duration-1000 ${
                escalationIndex >= 8 ? 'bg-severity-critical' :
                escalationIndex >= 6 ? 'bg-severity-high' :
                escalationIndex >= 4 ? 'bg-severity-medium' :
                'bg-severity-low'
              }`}
  ...
  function StatRow({ icon, label, value, highlight, capitalize }) {
    return (
      <div className="flex items-center justify-between">
        ...
        <span className={`text-sm font-semibold ${highlight ? 'text-accent-orange' : 'text-text-primary'} ${capitalize ? 'capitalize' : ''}`}>
          {value}
        </span>
      </div>
    );
  ...
  function VerificationBar({ icon, label, count, total, color }) {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-text-secondary w-6 text-right">{count}</span>
      </div>
    );
  }